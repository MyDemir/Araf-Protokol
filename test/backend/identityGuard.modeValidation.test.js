...
describe("identity normalization guard mode validation", () => {
  ...
  it("accepts valid modes: off/warn/enforce", () => {
    const { normalizeGuardMode } = require("../../backend/scripts/services/identityNormalizationGuard");
  ...
  it("rejects invalid mode instead of silently downgrading to warn", async () => {
    const { verifyIdentityNormalization } = require("../../backend/scripts/services/identityNormalizationGuard");
  ...
  it("keeps existing behavior for valid warn/enforce flows", async () => {
    ...
    jest.isolateModules(() => {
      jest.doMock("../../backend/scripts/utils/logger", () => logger);
      jest.doMock("../../backend/scripts/models/Order", () => Order);
      jest.doMock("../../backend/scripts/models/Trade", () => Trade);
      ({ verifyIdentityNormalization } = require("../../backend/scripts/services/identityNormalizationGuard"));
    });
