...
jest.mock("../../backend/scripts/models/Trade", () => ({ findOneAndUpdate: (...args) => mockTradeFindOneAndUpdate(...args) }));
jest.mock("../../backend/scripts/models/Order", () => ({ findOneAndUpdate: (...args) => mockOrderFindOneAndUpdate(...args) }));
...
describe("eventListener identity + env wiring", () => {
  ...
  function loadWorkerWithEnv(batchRaw, checkpointRaw, finalityRaw = "", nodeEnv = "test") {
    ...
    return require("../../backend/scripts/services/eventListener");
  }
  ...
  it("security_inferCryptoAssetFromToken_supports_legacy_env_with_warning", () => {
    ...
    const worker = loadWorkerWithEnv("", "");
    const mockLogger = require("../../backend/scripts/utils/logger");
