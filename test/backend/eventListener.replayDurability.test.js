...
jest.mock("../../backend/scripts/models/RewardFunding", () => ({ findOneAndUpdate: jest.fn() }));
jest.mock("../../backend/scripts/models/RewardEpoch", () => ({ findOneAndUpdate: jest.fn(), findOne: jest.fn(() => ({ lean: jest.fn().mockResolvedValue(null) })) }));
jest.mock("../../backend/scripts/models/RewardClaim", () => ({ findOneAndUpdate: jest.fn() }));
jest.mock("../../backend/scripts/models/RewardEpochAllocationEvent", () => ({ findOneAndUpdate: jest.fn().mockResolvedValue({ lastErrorObject: { updatedExisting: true } }) }));
