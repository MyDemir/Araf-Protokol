...
describe("PII taker-name identity guard + big id parsing", () => {
  ...
  it("returns 503 when identity normalization guard fails (no silent false-negative)", async () => {
    ...
    jest.isolateModules(() => {
      ...
      jest.doMock("../../backend/scripts/models/Trade", () => ({ findOne: jest.fn() }));
      jest.doMock("../../backend/scripts/models/User", () => ({ findOne: jest.fn() }));
      jest.doMock("../../backend/scripts/services/encryption", () => ({ decryptField: jest.fn(), decryptPayoutProfile: jest.fn() }));
