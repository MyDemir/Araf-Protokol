...
describe("reference-rates route", () => {
  ...
  it("returns informational semantics and fixed ticker set without arbitrary pair input", async () => {
    ...
    let router;
    jest.isolateModules(() => {
      jest.doMock("../../backend/scripts/middleware/rateLimiter", () => ({ marketReadLimiter }));
      jest.doMock("../../backend/scripts/services/referenceTicker", () => ({ getReferenceTickerPayload }));
      router = require("../../backend/scripts/routes/referenceRates");
    });
