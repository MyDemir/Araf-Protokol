...
describe("admin routes resilience + pagination semantics", () => {
  ...
  it("returns degraded summary payload instead of 500 when secondary deps fail", async () => {
    ...
    jest.isolateModules(() => {
      ...
      jest.doMock("../../backend/scripts/services/dlqProcessor", () => ({
      ...
      jest.doMock("../../backend/scripts/models/Feedback", () => ({ find: jest.fn(), countDocuments: jest.fn() }));
      router = require("../../backend/scripts/routes/admin");
    });
