...
export const buildAppViews = (ctx) => {
  ...
  const renderTradeRoom = () => {
    ...
    const burnExpiredDeadlinePassed = Boolean(activeTrade?.onchainId && roomState === 'CHALLENGED' && activeTrade.challengedAt && (new Date().getTime() - new Date(activeTrade.challengedAt).getTime() > 10 * 24 * 3600 * 1000));
    const panelActionConfig = (onClick, { disabled = false, disabledReasons = [] } = {}) => ({
      onClick,
    ...
    const tradeActionCallbacks = {
      report_payment: panelActionConfig(handleReportPayment, { disabled: isContractLoading }),
      release_funds: panelActionConfig(handleRelease, { disabled: isContractLoading || (isMaker && roomState === 'PAID' && !chargebackAccepted), disabledReasons: isMaker && roomState === 'PAID' && !chargebackAccepted ? [lang === 'TR' ? 'Chargeback onayı gerekli.' : 'Chargeback acknowledgement is required.'] : [] }),
      start_challenge: panelActionConfig(handleChallenge, { disabled: isContractLoading || makerChallengeBlocked, disabledReasons: makerChallengeBlocked ? [makerChallengeReason] : [] }),
      ping_maker: panelActionConfig(() => handlePingMaker(activeTrade.onchainId), { disabled: isContractLoading || !canPanelPingMaker, disabledReasons: !canPanelPingMaker ? [lang === 'TR' ? 'Maker uyarısı için grace period bekleniyor.' : 'Grace period must expire before pinging maker.'] : [] }),
      auto_release: panelActionConfig(() => handleAutoRelease(activeTrade.onchainId), { disabled: isContractLoading || !canPanelAutoRelease, disabledReasons: !canPanelAutoRelease ? [lang === 'TR' ? 'Auto-release için maker uyarısı sonrası 24 saat bekleyin.' : 'Wait 24h after maker ping before auto-release.'] : [] }),
      propose_cancel: panelActionConfig(() => {
      ...
      burn_expired: panelActionConfig(runBurnExpired, { disabled: isContractLoading || !burnExpiredDeadlinePassed, disabledReasons: !burnExpiredDeadlinePassed ? [lang === 'TR' ? '10 günlük yakma süresi henüz dolmadı.' : '10-day burn deadline has not passed.'] : [] }),
    };
