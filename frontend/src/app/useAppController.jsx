...
const handleCreateEscrow = async () => {
  ...
  // [TR] Legacy listing delete akışı V3'te deprecated.
  //      Bu handler bilinçli olarak kullanıcıyı yönlendirir; business logic değiştirmez.
  // [EN] Legacy listing delete flow is deprecated in V3.
  //      This handler intentionally provides UX guidance without changing business logic.
  const handleDeleteOrder = async (_order) => {
    setConfirmDeleteId(null);
    showToast(
      lang === 'TR'
        ? 'İlan silme akışı V3’te deprecated. Lütfen order cancel akışını kullanın.'
        : 'Listing delete is deprecated in V3. Please use the order cancel flow.',
      'info'
    );
  };
...
if (activeTrade?._pendingBackendSync && !activeTrade?.id) {
  ...
  return {
    ...
    // Render helpers preserved by name
    renderWalletModal,
    renderFeedbackModal,
    renderMakerModal,
    renderProfileModal,
