...
describe('frontend migration scaffold baseline', () => {
  it('keeps App.jsx as the PR #92 owner for session orchestration and contract handlers', () => {
    ...
    expect(appSource).toContain("import { SiweMessage } from 'siwe';");
    ...
    [
      ...
      expect(appSource, `${handlerName} should still be owned by App.jsx`).toContain(`const ${handlerName}`);
    });

    expect(appSource).toContain('new SiweMessage');
    expect(appSource).toContain('_pendingBackendSync: true');
    expect(appSource).toContain('setActiveTrade({ ...order, id: realTradeId, onchainId: onchainTradeId })');
  });
