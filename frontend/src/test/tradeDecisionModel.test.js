...
describe('buildTradeDecisionModel', () => {
  ...
  it('returns user-facing decision copy for all main state and role combinations', () => {
    ['LOCKED', 'PAID', 'CHALLENGED'].forEach((tradeState) => {
      ['maker', 'taker'].forEach((userRole) => {
        const model = buildTradeDecisionModel({ ...base, tradeState, userRole });
        const decisionText = [
          model.headline,
          model.subheadline,
          model.nowLabel,
          model.nowDescription,
          model.nextLabel,
          model.nextDescription,
        ];

        decisionText.forEach((value) => expect(value).toEqual(expect.any(String)));
        expect(decisionText.join(' ')).not.toMatch(/\b(LOCKED|PAID|CHALLENGED)\b/);
        expect(model.decisionSummary).toMatchObject({
          headline: model.headline,
          nowDescription: model.nowDescription,
          nextDescription: model.nextDescription,
        });
      });
    });
  });
  ...
  it('keeps stable action keys separate from localized user-facing labels', () => {
    ...
    const paidMaker = buildTradeDecisionModel({ ...base, tradeState: 'PAID', userRole: 'maker' });
    const paidTaker = buildTradeDecisionModel({ ...base, tradeState: 'PAID', userRole: 'taker' });
    const challenged = buildTradeDecisionModel({ ...base, tradeState: 'CHALLENGED', userRole: 'maker', canBurnExpired: true });
    ...
    expect(challenged.secondaryActions.map((a) => a.label)).not.toContain('burn_expired');
  });
