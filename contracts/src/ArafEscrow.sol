...
contract ArafEscrow is ReentrancyGuard, EIP712, Ownable, Pausable {
    ...
    function _executeCancel(uint256 _tradeId) internal {
        ...
        // 1. Eriyik Miktarı Hazineye Aktar (Mevcut Mantık)
        if (decayed > 0) {
            IERC20(t.tokenAddress).safeTransfer(treasury, decayed);
            emit BleedingDecayed(_tradeId, decayed, block.timestamp);
        }
        ...
        // Maker ücretini Maker'ın teminatından düş
        ...
        // Taker ücretini Taker'ın teminatından düş
        ...
        // 4. Transferleri gerçekleştir
        if (totalFeeToTreasury > 0) {
            IERC20(t.tokenAddress).safeTransfer(treasury, totalFeeToTreasury);
        }
        if (makerRefund > 0) {
            IERC20(t.tokenAddress).safeTransfer(t.maker, makerRefund);
        }
        if (takerRefund > 0) {
            IERC20(t.tokenAddress).safeTransfer(t.taker, takerRefund);
        }
