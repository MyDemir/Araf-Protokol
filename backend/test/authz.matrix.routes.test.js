...
function buildApp() {
  ...
  jest.doMock("../scripts/middleware/rateLimiter", () => new Proxy({}, { get: () => (_req, _res, next) => next() }));
  const emptyFind = () => ({ select: () => ({ lean: async () => null }) });
  jest.doMock("../scripts/models/User", () => ({ findOne: jest.fn(emptyFind), find: jest.fn(() => ({ select: () => ({ lean: async () => [] }) })) }));
  jest.doMock("../scripts/models/Order", () => ({ find: jest.fn(() => ({ select: () => ({ sort: () => ({ skip: () => ({ limit: () => ({ lean: async () => [] }) }) }) }) })), countDocuments: jest.fn(async () => 0) }));
  ...
  jest.doMock("../scripts/models/RevenueEvent", () => ({ find: jest.fn(() => ({ sort: () => ({ limit: () => ({ lean: async () => [] }) }) })) }));
  jest.doMock("../scripts/models/RewardEpoch", () => ({ countDocuments: jest.fn(async () => 0) }));
  jest.doMock("../scripts/models/RewardFunding", () => ({ countDocuments: jest.fn(async () => 0) }));
  jest.doMock("../scripts/models/RewardClaim", () => ({ countDocuments: jest.fn(async () => 0) }));
  jest.doMock("../scripts/services/protocolConfig", () => ({ getConfig: jest.fn(() => ({ tokenMap: {}, bondMap: {}, feeConfig: {}, cooldownConfig: {} })) }));
  jest.doMock("../scripts/services/health", () => ({ getReadiness: jest.fn(async () => ({ ok: true })) }));
  jest.doMock("../scripts/services/dlqProcessor", () => ({ getDlqMetrics: jest.fn(async () => ({ depth: 0 })) }));
  jest.doMock("../scripts/services/encryption", () => ({ decryptField: jest.fn(async () => JSON.stringify({ account_holder_name: "A" })), decryptPayoutProfile: jest.fn(async () => ({})), encryptField: jest.fn(async () => "enc") }));
  jest.doMock("../scripts/services/identityNormalizationGuard", () => ({ verifyIdentityNormalization: jest.fn(async () => true) }));

  const app = express();
  app.use(express.json());
  app.use(cookieParser());
  app.use("/api/auth", require("../scripts/routes/auth"));
  app.use("/api/orders", require("../scripts/routes/orders"));
  app.use("/api/trades", require("../scripts/routes/trades"));
  app.use("/api/pii", require("../scripts/routes/pii"));
  app.use("/api/receipts", require("../scripts/routes/receipts"));
  app.use("/api/admin", require("../scripts/routes/admin"));
  app.use("/api/logs", require("../scripts/routes/logs"));
  return app;
}
