...
describe("eventListener ReputationUpdated V3 handling", () => {
  beforeEach(() => {
    ...
    worker._fetchReputationFromChain = jest.fn().mockResolvedValue({
      successfulTrades: 10n,
      failedDisputes: 2n,
      bannedUntil: 0n,
      consecutiveBans: 3n,
      effectiveTier: 2n,
      ...
      disputeLossCount: 1n,
      riskPoints: 8n,
      lastPositiveEventAt: 1700000000n,
      lastNegativeEventAt: 1700000010n,
    });
  });
  ...
  it("uses consecutiveBans directly from ReputationUpdated event payload", async () => {
    ...
    expect(worker._fetchReputationFromChain).toHaveBeenCalledWith(
    ...
    expect(update.$set["reputation_breakdown.manual_release_count"]).toBe(5);
    expect(update.$set["reputation_breakdown.auto_release_count"]).toBe(2);
    expect(update.$set["reputation_breakdown.burn_count"]).toBe(1);
    expect(update.$set["reputation_breakdown.disputed_resolved_count"]).toBe(3);
    expect(update.$set["reputation_breakdown.dispute_win_count"]).toBe(2);
    expect(update.$set["reputation_breakdown.dispute_loss_count"]).toBe(1);
    expect(update.$set["reputation_breakdown.risk_points"]).toBe(8);
  });
});
