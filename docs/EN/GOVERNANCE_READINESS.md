...
|---|---|---|---|---|---|
| `contracts/scripts/deploy.js` — constructor `treasury`, then `setTokenConfig(...)`, then `transferOwnership(FINAL_OWNER_ADDRESS)` | Treasury ops for treasury; governance multisig for final owner | `CONFIRM_PUBLIC_DEPLOY=yes` on public chains; public/custom deploy must end with final owner separated from treasury | Constructor treasury or token config is wrong, or ownership remains with deployer/hot wallet | `TREASURY_ADDRESS`, `FINAL_OWNER_ADDRESS`, `CONFIRM_PUBLIC_DEPLOY`, token envs, decimals, tier max arrays, chain id, separation | `escrow.treasury()`, `escrow.owner()`, `getTokenConfig(...)`, deployment manifest, ownership transfer log |
| `contracts/scripts/deployRewards.js` — `ArafRevenueVault(..., finalTreasury, owner)`, `ArafRewards(..., owner)`, `vault.setRewards(...)`, `vault.setSupportedToken(...)` | Rewards governance/deploy ops; final treasury under treasury/accounting control | Vault/rewards deploy + initial wiring only; **no escrow treasury switch** | Wrong vault/rewards owner, wrong final treasury, wrong supported token, or accidental fresh public escrow deployment | `FINAL_OWNER_ADDRESS`, `FINAL_TREASURY_ADDRESS`, `ARAF_ESCROW_ADDRESS`, USDT/USDC envs, `CONFIRM_FRESH_ESCROW_DEPLOY`, manifest overwrite guard `CONFIRM_OVERWRITE_REWARDS_MANIFEST` | Manifest addresses, `vault.owner()`, `rewards.owner()`, `vault.finalTreasury()`, `vault.rewards()`, supported tokens, `rewardBps=4000` |
| `contracts/scripts/configureRewards.js` — `vault.setRewards(...)`, `vault.setSupportedToken(...)` | Rewards governance ops | Wiring-only post-deploy configure; treasury switch explicitly forbidden | Configure step accidentally becomes a treasury switch or wires wrong rewards/token addresses | Env/manifest addresses, no `CONFIRM_SWITCH_TREASURY_TO_VAULT`, owner authority for vault, token envs | `vault.rewards()`, `supportedToken(usdt/usdc)`, no `escrow.treasury()` change |
| `contracts/scripts/verifyRewardsDeployment.js` — read-only owner-readiness verification | Read-only ops | Go-live readiness verification, not an authority-changing transaction | Missing/mismatched owner, treasury, rewards, or token wiring goes unnoticed | Env/manifest addresses, optional `EXPECT_ESCROW_TREASURY_ADDRESS`, RPC/chain id | OK/fail outputs; `vault.owner`, `rewards.owner`, `rewardBps=4000`, token support, optional treasury match |
| `contracts/scripts/switchRewardsTreasury.js` — `escrow.setTreasury(vault)` | Final owner multisig | Separate explicit change window; `CONFIRM_TREASURY_SWITCH=true` | Escrow treasury switches to wrong vault; revenue redirect risk | Vault/rewards/escrow wiring, token support, `rewardBps`, `EXPECTED_CURRENT_TREASURY_ADDRESS`, smoke/verify complete | `escrow.treasury()==vault`, revenue-hook smoke, readiness log |
| `contracts/scripts/smokeRewards.js` — local `vault.setRewards(...)`, `vault.setSupportedToken(...)`, guard checks for `setRewardBps(...)` | Deploy/QA ops only | Validation-only; not used for production governance changes | Public smoke can mislead or mutate state if run with production owner credentials | Local/staging? if public, `CONFIRM_PUBLIC_SMOKE=yes`; confirm addresses/signers are not production governance addresses | Smoke output; confirm no production owner state changed |
