...
|---|---|---|---|---|---|
| `contracts/scripts/deploy.js` — constructor `treasury`, ardından `setTokenConfig(...)`, ardından `transferOwnership(FINAL_OWNER_ADDRESS)` | Treasury ops için treasury; final owner için governance multisig | Public chain'de `CONFIRM_PUBLIC_DEPLOY=yes`; public/custom deploy final owner ile treasury ayrılmış bitmeli | Constructor treasury veya token config yanlış olur ya da ownership deployer/hot wallet'ta kalır | `TREASURY_ADDRESS`, `FINAL_OWNER_ADDRESS`, `CONFIRM_PUBLIC_DEPLOY`, token env'leri, decimals, tier max array'leri, chain id, separation | `escrow.treasury()`, `escrow.owner()`, `getTokenConfig(...)`, deployment manifest, ownership transfer log |
| `contracts/scripts/deployRewards.js` — `ArafRevenueVault(..., finalTreasury, owner)`, `ArafRewards(..., owner)`, `vault.setRewards(...)`, `vault.setSupportedToken(...)` | Rewards governance/deploy ops; final treasury treasury/accounting kontrolünde | Sadece vault/rewards deploy + ilk wiring; **escrow treasury switch yok** | Yanlış vault/rewards owner, yanlış final treasury, yanlış supported token veya public ağda yanlışlıkla fresh escrow deploy | `FINAL_OWNER_ADDRESS`, `FINAL_TREASURY_ADDRESS`, `ARAF_ESCROW_ADDRESS`, USDT/USDC env'leri, `CONFIRM_FRESH_ESCROW_DEPLOY`, manifest overwrite guard `CONFIRM_OVERWRITE_REWARDS_MANIFEST` | Manifest adresleri, `vault.owner()`, `rewards.owner()`, `vault.finalTreasury()`, `vault.rewards()`, supported token'lar, `rewardBps=4000` |
| `contracts/scripts/configureRewards.js` — `vault.setRewards(...)`, `vault.setSupportedToken(...)` | Rewards governance ops | Deploy sonrası sadece wiring; treasury switch açıkça yasak | Configure adımı yanlışlıkla treasury switch'e dönüşür veya yanlış rewards/token adresleri wire edilir | Env/manifest adresleri, `CONFIRM_SWITCH_TREASURY_TO_VAULT` yokluğu, vault owner authority, token env'leri | `vault.rewards()`, `supportedToken(usdt/usdc)`, `escrow.treasury()` değişmedi |
| `contracts/scripts/verifyRewardsDeployment.js` — read-only owner-readiness doğrulaması | Read-only ops | Go-live readiness verification; authority değiştiren tx değildir | Eksik/mismatched owner, treasury, rewards veya token wiring fark edilmez | Env/manifest adresleri, optional `EXPECT_ESCROW_TREASURY_ADDRESS`, RPC/chain id | OK/fail çıktıları; `vault.owner`, `rewards.owner`, `rewardBps=4000`, token support, optional treasury match |
| `contracts/scripts/switchRewardsTreasury.js` — `escrow.setTreasury(vault)` | Final owner multisig | Ayrı explicit change window; `CONFIRM_TREASURY_SWITCH=true` | Escrow treasury yanlış vault'a geçer; revenue redirect riski | Vault/rewards/escrow wiring, token support, `rewardBps`, `EXPECTED_CURRENT_TREASURY_ADDRESS`, smoke/verify tamam | `escrow.treasury()==vault`, revenue hook smoke, readiness log |
| `contracts/scripts/smokeRewards.js` — local `vault.setRewards(...)`, `vault.setSupportedToken(...)`, `setRewardBps(...)` guard kontrolleri | Sadece deploy/QA ops | Validation-only; production governance değişikliği için kullanılmaz | Public smoke production owner credential'larıyla çalışırsa yanıltıcı olur veya state değiştirir | Local/staging mi; public ise `CONFIRM_PUBLIC_SMOKE=yes`; adres/signer'ların production governance adresleri olmadığı | Smoke çıktısı; hiçbir production owner state değişmediği doğrulaması |
