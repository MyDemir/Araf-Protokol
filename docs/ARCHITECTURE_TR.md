...
| Tier | TRY Limiti | Maker Teminatı | Taker Teminatı | Cooldown | **Erişim İçin Gerekli İtibar (On-Chain Zorunlu)** |
|---|---|---|---|---|---|
| **Tier 0** | 250 – 5.000 ₺ | %0 | %0 | 24 saat / işlem | **Varsayılan:** Tüm yeni kullanıcılar buradan başlar. |
| **Tier 1** | 5.001 – 50.000 ₺ | %8 | %10 | 24 saat / işlem | ≥ 15 başarılı işlem, **≤ 2 başarısız uyuşmazlık** |
| **Tier 2** | 50.001 – 250.000 ₺ | %6 | %8 | Sınırsız | ≥ 50 başarılı işlem, **≤ 5 başarısız uyuşmazlık** |
| **Tier 3** | 250.001 – 1.000.000 ₺ | %5 | %5 | Sınırsız | ≥ 100 başarılı işlem, **≤ 10 başarısız uyuşmazlık** |
| **Tier 4** | 1.000.001+ ₺ | %2 | %2 | Sınırsız | ≥ 200 başarılı işlem, **≤ 15 başarısız uyuşmazlık** |
...
| `pingMaker(tradeId)` | Taker'ın, 48 saatlik `GRACE_PERIOD` dolduktan sonra pasif kalan Maker'a "hayat sinyali" göndermesini sağlar. Bu, `autoRelease` fonksiyonunu çağırmak için bir ön koşuldur. |
...
| `burnExpired(tradeId)` | 10 günlük kanama süresi dolan işlemlerdeki tüm fonların Hazine'ye aktarılmasını sağlar. |
...
| `decayReputation(address)` | "Temiz Sayfa" kuralını on-chain'de uygular. Bir kullanıcının son yasağının üzerinden 180 gün geçtiyse, `consecutiveBans` (ardışık yasak) sayacını sıfırlar. **Önemli Not:** Bu fonksiyon, istismarı önlemek amacıyla `maxAllowedTier` (Tier Tavanı) cezasını sıfırlamaz. Kullanıcı, kaybettiği Tier seviyelerini sisteme yeniden dürüstçe katkıda bulunarak kazanmalıdır. |
...
### 9.5 Olay Dinleyici Güvenilirliği
